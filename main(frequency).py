#This file doesn't use the data loader py file yet. It runs quite slow!
...
class MedleyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        link = find_file(uuid4)

        # fs, audio = wavfile.read(link)
        timeseries_length = 3
        audio = np.zeros((timeseries_length, 14), dtype=np.float64)
        y, sr = librosa.load(link)

        mfcc = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=13)
        spectral_center = librosa.feature.spectral_centroid(y=y, sr=sr)

        audio[:, 0:13] = mfcc.T[0:timeseries_length, :]
        audio[:, 13:14] = spectral_center.T[0:timeseries_length, :]
